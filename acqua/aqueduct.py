...
fileGestori = 'resource/CatalogoEntiGestori.csv'
df_ = pd.read_csv(fileGestori,encoding='utf-8')
df = df_[df_['CODGESTORE'].notna()]
idx = df.groupby('CODGESTORE').count()['ENTE_GESTORE']
failIdx = idx[idx>1]
if len(failIdx)>1: raise Exception('Deplicate key in ENTE_GESTORE %s',failIdx)
df.set_index('CODGESTORE',inplace=True)
