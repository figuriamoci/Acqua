...
#georeferencingString: Data cleaning; rimozione termine 'mini acquedotto'
df['type'] = 'POINT'
#Datacleaning
df['georeferencingString'] = df['alias_address'].apply(lambda x: x.replace('mini acquedotto','').strip())
df['georeferencingString'] = df['alias_address'].apply(lambda x: x.replace('loc.','').strip())
